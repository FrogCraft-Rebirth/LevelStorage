...
public class TestParticleFX extends Particle {
	...
	public void renderParticle(Tessellator tessellator, float f, float f1,
		...
		tessellator.getBuffer().pos(0.0D, 0.0D, 0.0D).tex(1.0D, 0.0D);
		tessellator.getBuffer().pos(1.0D, 0.0D, 0.0D).tex(1.0D, 0.0D);
		tessellator.getBuffer().pos(1.0D, 1.0D, 0.0D).tex(0.0D, 1.0D);
		tessellator.getBuffer().pos(0.0D, 1.0D, 0.0D).tex(0.0D, 1.0D);
		tessellator.draw();
